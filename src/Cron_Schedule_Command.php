...
class Cron_Schedule_Command extends WP_CLI_Command {
	 ...
	 */
	protected function format_schedule( array $schedule, $name ) {
		$schedule['name'] = $name;
		return $schedule;
	}
	...
	*/
	protected function get_schedules() {
		...
		if ( ! empty( $schedules ) ) {
			uasort( $schedules, [ $this, 'sort' ] );
			$schedules = array_map( [ $this, 'format_schedule' ], $schedules, array_keys( $schedules ) );
		}
		return $schedules;
	}
	...
	private function get_formatter( &$assoc_args ) {
		return new Formatter( $assoc_args, self::FIELDS, 'schedule' );
	}
